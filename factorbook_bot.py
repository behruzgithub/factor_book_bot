...
from functions import home_buttons, inlines, text, insert, message_inline

# TOKEN = getenv("BOT_TOKEN")
TOKEN = '6780547956:AAFzpMFOywEyEJ-iOgi57ZJgfub7v8txUP4'
order_book_list = []
admins = [442722237]
dp = Dispatcher()
_c = 0

_names = ['⚡️IKAR', '📚 Factor books kitoblari', 'Biznes kitoblar', 'Diniy kitoblar', '📚 Boshqa kitoblar',
          'Psixologik kitoblar', 'Tarbiyaviy-oylaviy kitoblar', "Turk badiiy-mar'rifiy kitoblar", 'Badiiy Romanlar',
          'Qissa va Romanlar', 'Badiiy kitoblar va qissalar', '🔍 Qidirish']
...
async def books(message: Message):
    ...
    with engine.connect() as conn:
        res = conn.execute(Select(Basket))
        conn.commit()
...
async def state_name(message: Message, state: FSMContext):
    ...
    await message.answer("Kitobning kitob haqidagi malumot kiring kiriting")
...
async def state_name(message: Message, state: FSMContext):
    ...
    await message.answer("Kitobning kitobning qaysi categoryga tegishli ekanligini kiriting kiriting")
